/*
 * <cani-8x8-work-showcase>  --  VERSION V18.9 (V18.8 + Soft-phone start 02/04, 3.0s dwell, 400ms handoff; contract CC373E5B names this V18.8 - filename taken, versioning immutable) (V18.7 + my.plan editorial typography: heading curve 36-78px per band, grey line #A1A1A6, eyebrow 12/600/.14em, intro 17/18px, card 22/24px 600 + 16px body, CTA 16/600, proof 13.5px; scenes untouched) (V18.6 + my.plan black tab rail: #1F1F1F rail r12 p4, white active pill r9 with my.plan shadow, 13px/600 labels, active #0A0A0B / inactive #B9B9BE, no underline, no cell borders, four-across at all widths; everything else untouched)
 *
...
(function () {
  ...
  var CSS = [
    ...
    '.feature-panel{position:absolute;inset:0;padding:28px 30px;opacity:0;transform:translateY(12px);pointer-events:none;transition:opacity .14s linear,transform .4s cubic-bezier(.32,.72,0,1);}',
    '.feature-panel.active{opacity:1;transform:none;pointer-events:auto;transition:opacity .26s cubic-bezier(.32,.72,0,1) .14s,transform .4s cubic-bezier(.32,.72,0,1);}',
    ...
    '.feature-nav{width:100%;max-width:560px;margin-top:13px;padding:4px;background:#1F1F1F;border-radius:12px;display:grid;grid-template-columns:repeat(4,1fr);gap:0;}',
    '.feature-tab{min-height:36px;min-width:0;border:0;border-radius:9px;background:transparent;color:#B9B9BE;font-size:13px;font-weight:600;letter-spacing:-0.01em;cursor:pointer;padding:9px 4px;transition:background .25s,color .25s,box-shadow .25s;}',
    '.feature-tab[aria-selected="true"]{background:#fff;color:#0A0A0B;box-shadow:0 2px 6px rgba(0,0,0,0.34),0 0 0 0.5px rgba(255,255,255,0.10);}',
    ...
    '.scene{position:absolute;inset:4%;z-index:2;opacity:0;transform:translateY(14px) scale(.985);pointer-events:none;transition:opacity .4s cubic-bezier(.32,.72,0,1),transform .4s cubic-bezier(.32,.72,0,1);}',
    ...
    '@media (max-width:767px){h2{font-size:clamp(35px,10vw,46px);line-height:1}.stage-shell{width:112%;aspect-ratio:.92/1}.reference-frame{overflow:hidden}.motion-phone{left:3%;top:8%;width:38%;height:78%;border-radius:15px}.feature-card{border-radius:8px}.feature-nav{border-radius:12px}.feature-tab{min-height:44px;font-size:12px}.motion-toggle{min-height:44px}.meeting-board-demo{inset:7% 2%}.board-demo-grid{grid-template-columns:40px repeat(3,1fr);padding:18px 8px}.board-demo-grid i:nth-last-child(-n+3){display:none}.conversation-status{right:3%}}',
    '',
    ...
    '@media (max-width:767px){.feature-nav{grid-template-columns:repeat(4,1fr);}}',
  ...
  var TYPO_CSS =
    '/* ==== V18.8 editorial typography = my.plan authority (served measured curve). Appended last: wins the layered legacy cascade. Scenes excluded. ==== */' +
    'h2{font-size:clamp(36px,10.5vw,44px);line-height:.97;font-weight:600;letter-spacing:-.042em;color:#0a0a0b;}' +
    '@media (min-width:768px){h2{font-size:46px;}}' +
    '@media (min-width:1024px){h2{font-size:5vw;}}' +
    '@media (min-width:1280px){h2{font-size:64px;}}' +
    '@media (min-width:1440px){h2{font-size:4.5vw;}}' +
    '@media (min-width:1600px){h2{font-size:74px;}}' +
    '@media (min-width:1920px){h2{font-size:clamp(74px,4vw,78px);}}' +
    'h2 span{color:#a1a1a6;}' +
    '.eyebrow{font-size:12px;font-weight:600;letter-spacing:.14em;color:#5f5f63;}' +
    '.intro{font-size:17px;line-height:1.52;letter-spacing:-.012em;color:#6e6e73;}' +
    '@media (min-width:768px){.intro{font-size:18px;}}' +
    '.feature-panel h3{font-size:22px;line-height:1.14;font-weight:600;letter-spacing:-.03em;color:#0a0a0b;}' +
    '@media (min-width:768px){.feature-panel h3{font-size:24px;}}' +
    '.feature-panel p{font-size:16px;line-height:1.5;letter-spacing:-.01em;color:#6e6e73;}' +
    '.cta{font-size:16px;font-weight:600;letter-spacing:-.012em;}' +
    '.proof{font-size:13.5px;letter-spacing:-.01em;}';
    this.shadowRoot.innerHTML = '<style>' + CSS + RIGHTS_SAFE_CSS + TYPO_CSS + '</style>' + HTML;
    ...
    this._idx = 1; /* V18.9: Soft phone start (02/04) */
    this._paused = false;
