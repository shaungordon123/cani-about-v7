 ...
 *
 * MOTION-V1, 2026-08-07 - adds a one-shot viewport-triggered entrance reveal.
 * Additive only: the 79KB markup string is untouched. Groups are tagged from
 * JS at connect time and the stylesheet keys off those tags.
 * Built from Velo baseline 79418 B / E119853D... by build-myplan-motion.mjs.
 *
 * REVISION v2 (MYPLAN-L1-R3), 2026-07-27 - Shaun-directed staging design
 * revision under the authorised Layer-2 scope. Derived from the accepted
 * Layer-1 baseline (SHA 950dc8a3...) which remains untouched. Changes:
 *   1. Pure #FFFFFF background on :host and [data-root]; the grey/vignette
 *      radial background layer REMOVED so the section reads as one continuous
 *      white area with Networks We Work With (measured: the networks section
 *      is transparent over the white page background).
 *   2. :host width:100%/max-width:none for genuine full-bleed placement
 *      (the Wix-side fix is disabling the section's "Apply max width" grid).
 *   3. Responsive compact-scale token system (CSS variables on [data-root]):
 *      heading/body scale, spacing rhythm and a device-stage `zoom` per
 *      breakpoint band (>=1600 / 1151-1599 / <=1150 / <=767). `zoom` affects
 *      layout and composes with the drift/emphasis/tilt transforms - no
 *      transform:scale() on the host, no auto-height breakage.
 *   4. Benefit-panel minimum height is now the --panel-min token (JS reads it
 *      in _fitPanel), reduced in step with the type scale.
 * All copy, cards, states, interactions, CTAs, accessibility and motion
 * behaviour are unchanged. Fidelity exceptions (Instrument Sans, #FF1478)
 * stand. U-3/U-4 remain open live-publication gates.
 *
 * LAYER 1 PRODUCTION PREPARATION (original baseline notes follow).
 *
...
(function () {
  ...
  /* ------------------------------------------------------------------ */
  var CSS = '' +
    ':host{display:block;width:100%;max-width:none;background:#FFFFFF;}' +
    ...
    '[data-root]{' +
      ...
      /* v2 compact-scale tokens - desktop PC (>=1600px) defaults. */
      '--h1:clamp(74px,4vw,78px);--h3:24px;--body-lg:18px;--body:16px;--cta:16px;' +
      '--m-head:20px;--m-intro:22px;--m-panel:36px;--m-seg:14px;--m-cta:32px;--m-proof:22px;' +
      '--pad-y:72px;--pad-x:88px;--gap:112px;--panel-min:210;--stage-zoom:0.8075;}' +
    '[data-stage]{zoom:var(--stage-zoom);}' +
    /* Laptop band */
    '@media (max-width:1599px){[data-root]{--h1:clamp(64px,4.5vw,72px);--stage-zoom:0.798;--gap:104px;}}' +
    /* Tablet band (also the single-column collapse range) */
    '@media (max-width:1150px){[data-root]{--h1:clamp(46px,5vw,56px);--stage-zoom:0.836;--pad-y:56px;}}' +
    /* Mobile band */
    '@media (max-width:767px){[data-root]{--h1:clamp(36px,10.5vw,44px);--h3:22px;--body-lg:17px;--stage-zoom:0.855;--pad-y:48px;--panel-min:0;}}' +
    ...
    '@media (max-width:880px){[data-seg]{min-height:44px;}[data-cta="text"]{min-height:44px;display:inline-flex!important;align-items:center;}}@media (max-width:767px){' +
    ...
    '[data-root]{--rv-ease:cubic-bezier(0.22,1,0.36,1);--rv-ease-soft:cubic-bezier(0.22,1,0.36,1);--rv-copy:700ms;--rv-panel:780ms;--rv-stage:850ms;--rv-travel:26px;--rv-travel-stage:20px;}:host([data-rv=\'armed\']) [data-rvg]{opacity:0;transition:none!important;}:host([data-rv=\'armed\']) [data-rvg=\'copy\']{transform:translateY(var(--rv-travel));}:host([data-rv=\'armed\']) [data-rvg=\'panel\']{transform:translateY(calc(var(--rv-travel) + 4px));}:host([data-rv=\'armed\']) [data-rvg=\'stage\']{transform:translateY(var(--rv-travel-stage)) scale(.988);}:host([data-rv=\'armed\']) [data-rvg=\'float\']{transform:none;}:host([data-rv=\'revealing\']) [data-rvg],:host([data-rv=\'revealed\']) [data-rvg]{opacity:1;transform:none;}:host([data-rv=\'revealing\']) [data-rvg=\'copy\']{transition:opacity var(--rv-copy) var(--rv-ease) var(--rv-d),transform var(--rv-copy) var(--rv-ease) var(--rv-d);}:host([data-rv=\'revealing\']) [data-rvg=\'panel\']{transition:opacity var(--rv-panel) var(--rv-ease-soft) var(--rv-d),transform var(--rv-panel) var(--rv-ease-soft) var(--rv-d);}:host([data-rv=\'revealing\']) [data-rvg=\'stage\']{transition:opacity var(--rv-stage) var(--rv-ease) var(--rv-d),transform var(--rv-stage) var(--rv-ease) var(--rv-d);}:host([data-rv=\'revealing\']) [data-rvg=\'float\']{transition:opacity 620ms var(--rv-ease-soft) var(--rv-d);}@media (max-width:767px){[data-root]{--rv-copy:560ms;--rv-panel:650ms;--rv-stage:680ms;--rv-travel:20px;--rv-travel-stage:14px;}}@media(prefers-reduced-motion:reduce){:host([data-rv]) [data-rvg]{opacity:1!important;transform:none!important;transition:none!important;}}';
  ...
  '<div data-root style="position:relative;overflow:hidden;background:#fff;">' +
    ...
    '<section data-exp aria-label="The my.plan platform" style="position:relative;z-index:1;display:grid;grid-template-columns:1.02fr 1fr;gap:var(--gap);max-width:1520px;margin:0 auto;padding:0 var(--pad-x);align-items:center;min-height:calc(100vh - 116px);">' +

      '<div data-acts style="padding:var(--pad-y) 0;display:flex;flex-direction:column;align-items:flex-start;">' +
        ...
        '<h2 style="font-size:var(--h1);line-height:0.97;font-weight:600;letter-spacing:-0.042em;margin:var(--m-head) 0 0;max-width:680px;color:#0A0A0B;">' +
          'Mobile device controls,<br><span style="color:#A1A1A6;">analytics &amp; tracking.</span>' +
        '</h2>' +

        '<p style="font-size:var(--body-lg);line-height:1.52;color:#6E6E73;margin:var(--m-intro) 0 0;max-width:540px;font-weight:400;letter-spacing:-0.012em;text-wrap:pretty;">' +
        ...
        '<div data-slidewrap style="position:relative;width:100%;max-width:580px;min-height:238px;margin-top:var(--m-panel);border-radius:22px;background:#fff;border:1px solid rgba(10,10,11,0.07);box-shadow:0 1px 2px rgba(10,10,11,0.04),0 26px 54px -26px rgba(10,10,11,0.28);overflow:hidden;">' +
          ...
          '<div data-slide="0" role="tabpanel" id="mp-panel-0" aria-labelledby="mp-tab-0" style="position:absolute;inset:0;padding:32px 34px;transition:opacity 0.6s cubic-bezier(0.32,0.72,0,1),transform 0.6s cubic-bezier(0.32,0.72,0,1);opacity:1;">' +
            ...
            '</div>' +
            '<h3 style="font-size:var(--h3);line-height:1.14;font-weight:600;letter-spacing:-0.03em;color:#0A0A0B;margin:20px 0 0;">Every call, text and megabyte, as it happens.</h3>' +
            '<p style="font-size:var(--body);line-height:1.5;color:#6E6E73;margin:11px 0 0;letter-spacing:-0.01em;text-wrap:pretty;">Live usage across your estate the moment it lands &mdash; no surprises at the end of the month.</p>' +
          '</div>' +
          ...
          '<div data-slide="1" role="tabpanel" id="mp-panel-1" aria-labelledby="mp-tab-1" aria-hidden="true" style="position:absolute;inset:0;padding:32px 34px;transition:opacity 0.6s cubic-bezier(0.32,0.72,0,1),transform 0.6s cubic-bezier(0.32,0.72,0,1);opacity:0;transform:translateY(14px);pointer-events:none;">' +
            ...
            '</div>' +
            '<h3 style="font-size:var(--h3);line-height:1.14;font-weight:600;letter-spacing:-0.03em;color:#0A0A0B;margin:20px 0 0;">Know where every device is, right now.</h3>' +
            '<p style="font-size:var(--body);line-height:1.5;color:#6E6E73;margin:11px 0 0;letter-spacing:-0.01em;text-wrap:pretty;">Connected devices on a live map alongside their data use &mdash; and lost handsets recovered in moments.</p>' +
          '</div>' +
          ...
          '<div data-slide="2" role="tabpanel" id="mp-panel-2" aria-labelledby="mp-tab-2" aria-hidden="true" style="position:absolute;inset:0;padding:32px 34px;transition:opacity 0.6s cubic-bezier(0.32,0.72,0,1),transform 0.6s cubic-bezier(0.32,0.72,0,1);opacity:0;transform:translateY(14px);pointer-events:none;">' +
            ...
            '</div>' +
            '<h3 style="font-size:var(--h3);line-height:1.14;font-weight:600;letter-spacing:-0.03em;color:#0A0A0B;margin:20px 0 0;">See exactly what is driving the data.</h3>' +
            '<p style="font-size:var(--body);line-height:1.5;color:#6E6E73;margin:11px 0 0;letter-spacing:-0.01em;text-wrap:pretty;">Top users and the apps eating allowances, so tariff and bolt-on decisions rest on real numbers.</p>' +
          '</div>' +
          ...
          '<div data-slide="3" role="tabpanel" id="mp-panel-3" aria-labelledby="mp-tab-3" aria-hidden="true" style="position:absolute;inset:0;padding:32px 34px;transition:opacity 0.6s cubic-bezier(0.32,0.72,0,1),transform 0.6s cubic-bezier(0.32,0.72,0,1);opacity:0;transform:translateY(14px);pointer-events:none;">' +
            ...
            '</div>' +
            '<h3 style="font-size:var(--h3);line-height:1.14;font-weight:600;letter-spacing:-0.03em;color:#0A0A0B;margin:20px 0 0;">From one mobile to hundreds, in your hands.</h3>' +
            '<p style="font-size:var(--body);line-height:1.5;color:#6E6E73;margin:11px 0 0;letter-spacing:-0.01em;text-wrap:pretty;">Services, bolt-ons and barring adjusted in a couple of taps, applied instantly across the estate.</p>' +
          '</div>' +
        '</div>' +

        '<div data-segwrap style="width:100%;max-width:580px;margin-top:var(--m-seg);background:#1F1F1F;border-radius:12px;padding:4px;">' +
          '<div data-tablist role="tablist" aria-label="my.plan platform benefits" style="position:relative;display:flex;">' +
            '<div data-thumb aria-hidden="true" style="position:absolute;top:0;bottom:0;left:0;width:25%;background:#fff;border-radius:9px;box-shadow:0 2px 6px rgba(0,0,0,0.34),0 0 0 0.5px rgba(255,255,255,0.10);transition:transform 0.5s cubic-bezier(0.32,0.72,0,1);"></div>' +
            ...
            '<button data-seg="1" type="button" role="tab" id="mp-tab-1" aria-controls="mp-panel-1" aria-selected="false" tabindex="-1" style="position:relative;flex:1;background:none;border:none;cursor:pointer;font-family:inherit;font-size:13px;font-weight:600;letter-spacing:-0.01em;padding:9px 4px;color:#B9B9BE;">Location</button>' +
            '<button data-seg="2" type="button" role="tab" id="mp-tab-2" aria-controls="mp-panel-2" aria-selected="false" tabindex="-1" style="position:relative;flex:1;background:none;border:none;cursor:pointer;font-family:inherit;font-size:13px;font-weight:600;letter-spacing:-0.01em;padding:9px 4px;color:#B9B9BE;">Insights</button>' +
            '<button data-seg="3" type="button" role="tab" id="mp-tab-3" aria-controls="mp-panel-3" aria-selected="false" tabindex="-1" style="position:relative;flex:1;background:none;border:none;cursor:pointer;font-family:inherit;font-size:13px;font-weight:600;letter-spacing:-0.01em;padding:9px 4px;color:#B9B9BE;">Control</button>' +
          '</div>' +
        '</div>' +

        '<div style="display:flex;align-items:center;gap:28px;margin-top:var(--m-cta);flex-wrap:wrap;">' +
          '<a data-cta="primary" href="/free-bill-review#enquiry" style="display:inline-block;background:#FF1478;color:#fff;font-weight:600;font-size:var(--cta);letter-spacing:-0.012em;padding:15px 28px;border-radius:12px;cursor:pointer;box-shadow:0 12px 32px -10px rgba(255,20,120,0.6);">Get a free telecoms review</a>' +
          '<a data-cta="text" href="/contact" style="display:inline-block;color:#FF1478;font-weight:600;font-size:var(--cta);letter-spacing:-0.012em;padding:4px 0;cursor:pointer;">Book a demo <span data-chev>&#8250;</span></a>' +
        '</div>' +

        '<div data-proof style="display:flex;align-items:center;gap:9px;margin-top:var(--m-proof);font-size:13.5px;color:#5F5F63;font-weight:400;letter-spacing:-0.01em;">' +
  ...
  proto.connectedCallback = function () {
    ...
    try { rvInstall(this, this.shadowRoot); } catch (e) {}
  ...
  proto._fitPanel = function () {
    ...
    /* v2: the panel floor is the --panel-min token (per breakpoint band). */
    var floor = 238;
    if (this._root) {
      var t = parseFloat(getComputedStyle(this._root).getPropertyValue('--panel-min'));
      if (!isNaN(t)) floor = t;
    }
    wrap.style.minHeight = Math.max(floor, tallest + 2) + 'px';
  };
  ...
  proto._show = function (i, opts) {
    ...
    this._segs.forEach(function (button, buttonIndex) {
      ...
      button.style.color = active ? '#0A0A0B' : '#B9B9BE';
  ...
  /*
   * Scroll-reveal for my.plan. Same state model and fail-open contract as the
   * accepted divider reference implementation:
   *   static | armed | revealing | revealed
   * with a committed paint boundary between armed and revealing, no page-load
   * timer, and a proximity-gated watchdog.
   */
  function rvGroups(root) {
    // Ordered logical groups. Index drives the stagger, so the hierarchy reads
    // as copy -> panel -> controls -> stage rather than everything at once.
    var g = [];
    var acts = root.querySelector('[data-acts]');
    var push = function (el, kind) { if (el && g.indexOf(el) === -1) { el.setAttribute('data-rvg', kind); g.push(el); } };
    if (acts) {
      var kids = acts.children;
      if (kids[0]) push(kids[0], 'copy');                       // eyebrow
      push(acts.querySelector('h2'), 'copy');                   // headline
      push(acts.querySelector('p'), 'copy');                    // intro
      push(acts.querySelector('[data-slidewrap]'), 'panel');    // feature card
      push(acts.querySelector('[data-segwrap]'), 'panel');      // tabs
      var primary = acts.querySelector('[data-cta="primary"]');
      if (primary && primary.parentElement && primary.parentElement !== acts) push(primary.parentElement, 'panel');
      push(acts.querySelector('[data-proof]'), 'panel');
    }
    // D-MOTION-03: [data-stage] carries an INLINE transition:transform 0.6s.
    // Inline beats any stylesheet rule, and because it names only transform,
    // opacity had no transition and snapped to 1 on the first frame. Forcing it
    // with !important would mean seizing a property the my.plan engine declares
    // inline, so instead the entrance moves to a wrapper and the stage keeps its
    // own transition untouched. The wrapper takes the stage's place as the grid
    // item; [data-exp] centres its items, so the wrapper hugs the stage and the
    // cell geometry is unchanged (asserted at 0.00px by the geometry gate).
    var stage = root.querySelector('[data-stage]');
    if (stage && stage.parentElement) {
      var sw = document.createElement('div');
      sw.setAttribute('data-rv-stagewrap', '');
      stage.parentElement.insertBefore(sw, stage);
      sw.appendChild(stage);
      push(sw, 'stage');
    }
    var floats = root.querySelectorAll('[data-float]');
    for (var i = 0; i < floats.length; i++) push(floats[i], 'float');
    return g;
  }
  ...
  // ago, this one waits for the remainder, capped at 400ms. Same-key calls
  // never wait (a section must not delay itself). Any failure -> zero delay.
  ...
  function rvInstall(host, root) {
    ...
    if (reduced) { host.setAttribute('data-rv', 'static'); return; }
    ...
    // Never hide something already on screen.
    if (rvInZone(host)) { host.setAttribute('data-rv', 'static');
      host.setAttribute('data-rv-reason', 'in-view-at-load'); return; }

    var groups = rvGroups(root);
    if (!groups.length) { host.setAttribute('data-rv', 'static'); return; }
    // Stagger by group index: 70ms desktop, tightened by the mobile media query
    // through the shared duration tokens.
    var STEP = 70;
    for (var i = 0; i < groups.length; i++) {
      var kind = groups[i].getAttribute('data-rvg');
      // floats trail the stage rather than competing with the copy
      var slot = kind === 'float' ? 7 + i * 0.35 : i;
      groups[i].style.setProperty('--rv-d', Math.round(slot * STEP) + 'ms');
    }

    var done = false, obs = null, grace = 0, tick = 0, guard = 0, finish = 0, onScroll = null;
    var settle = function () {
      ...
      var toRevealing = function () {
        ...
        var gateWait = rvGate('myplan', 850);
        if (gateWait > 0) {
          setTimeout(function () {
            if (host.getAttribute('data-rv') !== 'armed') return;
            host.setAttribute('data-rv', 'revealing');
            try { setTimeout(function () { host.setAttribute('data-rv', 'revealed'); }, 2200); }
            catch (e) { host.setAttribute('data-rv', 'revealed'); }
          }, gateWait);
          return;
        }
        host.setAttribute('data-rv', 'revealing');
        try { finish = setTimeout(function () { host.setAttribute('data-rv', 'revealed'); }, 2200); }
        catch (e) { host.setAttribute('data-rv', 'revealed'); }
      };
      try {
